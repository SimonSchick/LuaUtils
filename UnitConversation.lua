...
local function addMetricValues(prefix, targetTable, baseValue)
	baseValue = baseValue or 1
	targetTable[prefix] = baseValue
	for metric, factor in next, metric do
		targetTable[prefix .. metric] = baseValue * factor
	end
end
...
addMetricValues("meter", distances)
